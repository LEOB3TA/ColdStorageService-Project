...
public class WSHandler extends AbstractWebSocketHandler implements IWsMsgHandler {
    ...
    public void handleMessage(String s) {
        CommUtils.outgreen("WsMessageHandler | handleMessage:" + s);
        TextMessage message= new TextMessage(s);
        Iterator<WebSocketSession> iter = sessions.iterator();
        while( iter.hasNext() ){
            try{
                WebSocketSession session = iter.next();
                ColorsOut.outappl("WebSocketHandler | handleMsg " +
                        message.getPayload() + " for session " + session.getRemoteAddress() , ColorsOut.MAGENTA);
                //synchronized(session){
                session.sendMessage(message);
                //}
